...
namespace Trabalho_Pratico_2
{
    public class Enemy
    {
        ...
        public Enemy(Texture2D texture, Vector2 position, Animation animation, int groundLevel, List<Platform> platforms, SoundEffect deathSound, SoundEffect hurtSound)
        {
            Texture = texture;
            Position = position;
        ...
        public void TakeDamage(Vector2 attackerPosition, float force)
        {
            ...
            if (!isDamaged)
            {
                ...
                hurtSound?.Play(); // <-- ADICIONADO AQUI
